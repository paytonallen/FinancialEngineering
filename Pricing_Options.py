...
def BlackScholesModel(r,S,K,T,sigma,type='c'):
    """
    Calculating the price of an option using Black Scholes Model
    """
...
def gamma(r,S,K,T,sigma,type='c'):
    """
    Calculating the gamma of an option using Black Scholes Model
    normal distribution
    """
    d1 = (np.log(S/K)+(r+sigma**2/2)*T)/(sigma*np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
    try:
        gamma_calc = norm.cdf(d1,0,1)/(S*sigma*np.sqrt(T))
...
def vega_calc(r,S,K,T,sigma,type='c'):
    """
    Calculating the vega of an option using Black Scholes Model
    """
    d1 = (np.log(S/K)+(r+sigma**2/2)*T)/(sigma*np.sqrt(T))
    d2 = d1 - sigma * np.sqrt(T)
    try:
        vega_calc = S*norm.cdf(d1,0,1)*np.sqrt(T)
...
def theta_calc(r,S,K,T,sigma,type='c'):
    ...
    try:
        if type == 'c':
            theta_calc = -S*norm.cdf(d1,0,1)*sigma/(2*np.sqrt(T)) - r*K*np.exp(-r*T)*norm.cdf(d2,0,1)
        elif type == 'p':
            theta_calc = -S*norm.cdf(d1,0,1)*sigma/(2*np.sqrt(T)) + r*K*np.exp(-r*T)*norm.cdf(-d2,0,1)
